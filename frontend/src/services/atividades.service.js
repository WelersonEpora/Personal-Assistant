...
// combinação de filtros. Params: de, ate ("AAAA-MM-DD"), aluno_id, membro_id,
// tipo ("atendimento" | "avaliacao_fisica"), somente_confirmados (bool).
async function obter({ de, ate, alunoId, membroId, tipo, somenteConfirmados } = {}) {
  const params = {}
  if (de) params.de = de
  if (ate) params.ate = ate
  if (alunoId) params.aluno_id = alunoId
  if (membroId) params.membro_id = membroId
